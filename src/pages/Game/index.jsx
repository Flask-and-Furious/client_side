...
useEffect(() => {
  ...
  const [currentCodePackage, setCurrentCodePackage] = useState(codeLanguage
 == 'python' ? pythonFetchedCodePackages[progress] : javascriptFetchedCodePackages[progress])
  ...
  useEffect(() => { // update and reset information when jumping to the next question
    setCurrentCodePackage(codeLanguage == 'python' ? pythonFetchedCodePackages[progress] : javascriptFetchedCodePackages[progress])
    setIsAnswered(false)
    setIsCorrect(false)
  ...
  const submitCode = async () => {
    ...
    await axios.post(codeLanguage == 'python' ? 'http://127.0.0.1:5000/code' : 'http://localhost:3000/code', {"code-package": currentCodePackage}) // send codes to backend for processing
      .then(data => {   // data.data will contain the debugged function return values in an array
        ...
        if (data.data[0] === currentCodePackage['snippet']['return-1'] &&
            data.data[1] === currentCodePackage['snippet']['return-2'] ) { // we compare the incoming values with the saved return values from the database
